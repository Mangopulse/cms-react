...
(function (e, c) {
    ...
    f.prototype.apply_settings = function () {
        ...
        this.$element.after('<label class="file-label" data-title="' + this.settings.btn_choose + '"><span class="file-name" data-title="' + this.settings.no_file + '">' + (this.settings.no_icon ? '<i class="' + this.settings.no_icon + '"></i>' : "") + "</span></label>" + (k ? '<a class="notajax remove" href="#"><i class="' + this.settings.icon_remove + '"></i></a>' : ""));
...
(function (a, b) {
    a.fn.ace_wizard = function (c) {
        this.each(function () {
            ...
            var f = e.data("fu.wizard");
            f.$prevBtn.remove();
            f.$nextBtn.remove();
            f.$prevBtn = d.find(".btn-prev").eq(0).click(function () {
                e.wizard("previous");
            }).attr("disabled", "disabled");
            f.$nextBtn = d.find(".btn-next").eq(0).click(function () {
                e.wizard("next");
            }).removeAttr("disabled");
    ...
    a.fn.ace_colorpicker = function (c) {
        ...
        this.each(function () {
            ...
            a(this).hide().find("option").each(function () {
                ...
                e += '<li><a class="notajax ' + h + '" href="#" style="background-color:' + this.value + ';" data-color="' + this.value + '"></a></li>';
            }).end().on("change.ace_inner_call", function () {
                a(this).next().find(".btn-colorpicker").css("background-color", this.value);
            }).after('<div class="dropdown dropdown-colorpicker"><a data-toggle="dropdown" class="notajax dropdown-toggle" href="#"><span class="btn-colorpicker" style="background-color:' + f + '"></span></a><ul class="dropdown-menu' + (d.caret ? " dropdown-caret" : "") + (d.pull_right ? " pull-right" : "") + '">' + e + "</ul></div>").next().find(".dropdown-menu").on(ace.click_event, function (j) {
                var h = a(j.target);
        ...
        this.each(function () {
            ...
            for (var n in f) {
                if (f.hasOwnProperty(n)) {
                    ...
                    switch (p.name) {
                        case "font":
                            r += ' <a class="notajax btn btn-sm ' + q + ' dropdown-toggle" data-toggle="dropdown" title="' + p.title + '"><i class="' + p.icon + '"></i><i class="icon-angle-down icon-on-right"></i></a> ';
                            r += ' <ul class="dropdown-menu dropdown-light">';
                            for (var j in p.values) {
                                if (p.values.hasOwnProperty(j)) {
                                    r += ' <li><a class="notajax" data-edit="fontName ' + p.values[j] + '" style="font-family:\'' + p.values[j] + "'\">" + p.values[j] + "</a></li> ";
                                }
                            }
                            r += " </ul>";
                            break;
                        case "fontSize":
                            ...
                            for (var t in p.values) {
                                if (p.values.hasOwnProperty(t)) {
                                    r += ' <li><a class="notajax" data-edit="fontSize ' + t + '"><font size="' + t + '">' + p.values[t] + "</font></a></li> ";
                                }
                            }
                            r += " </ul> ";
                            break;
                        case "createLink":
                            r += ' <div class="inline position-relative"> <a class="notajax btn btn-sm ' + q + ' dropdown-toggle" data-toggle="dropdown" title="' + p.title + '"><i class="' + p.icon + '"></i></a> ';
                        ...
                        case "backColor":
                            ...
                            break;
                        case "viewSource":
                            r += ' <a class="notajax btn btn-sm ' + q + '" data-view="source" title="' + p.title + '"><i class="' + p.icon + '"></i></a> ';
                            break;
                        default:
                            r += ' <a class="notajax btn btn-sm ' + q + '" data-edit="' + p.name + '" title="' + p.title + '"><i class="' + p.icon + '"></i></a> ';
                            break;
                    }
                }
            }
