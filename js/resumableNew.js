...
(function() {
    ...
    var Resumable = function(opts) {
        ...
        $.defaults = {
            ...
            imageSizeErrorCallback: function(file, errorCount) {
                alert(file.fileName || file.name + ' is too big. Please use images less than 3200x3200px ' + $.getOpt('fileType') + '.');
            }
        };
        $.opts = opts || {};
        $.getOpt = function(o) {
            ...
            $h.each(fileList, function(file) {
                ...
                try {
                    ...
                    image.onload = function () {
                        //if (image.height > 3200 || image.width > 3200) {
                        //    alert(file.fileName || file.name + ' is too big. Please use images less than 3200x3200px.');
                        //    $.files.pop(f);
                        //    files.pop(f);
                        //    return false;
                        //}
        ...
        function ResumableChunk(resumableObj, fileObj, offset, callback) {
            ...
            $.test = function() {
                ...
                params.push(['resumableFilter', encodeURIComponent($.getOpt('filterType'))].join('='));
                params.push(['appKey', encodeURIComponent($.getOpt('appKey'))].join('='));
                params.push(['scope', encodeURIComponent($.getOpt('scope'))].join('='));
            ...
            $.send = function () {
                ...
                var query = {
                    ...
                    resumableFilter: $.getOpt('filterType'),
                    appKey: $.getOpt('appKey'),
                    scope: $.getOpt('scope'),
                };
                // Mix in custom data
                var customQuery = $.getOpt('query');
                ...
                $.xhr.setRequestHeader("Authorization", "basic " + btoa(JS_CONFIG.UploadToken));
                $.xhr.send(data);
            };
            $.abort = function() {
                // Abort and reset
        ...
        // QUEUE
        $.uploadNextChunk = function(message) {
            var found = false;
        ...
        $.assignBrowse = function(domNodes, isDirectory) {
            ...
            $h.each(domNodes, function(domNode) {
                ...
                input.setAttribute('accept', $.getOpt('filter'));
                if (isDirectory) {
