    ...
    Tag.prototype = {
        ...
        , show: function () {
            ...
            that.input = $('<input type="text">')
                ...
                .on('keydown',
                    function(event) {
                        if (event.originalEvent.key == "," || event.keyCode == 13 || event.keyCode == 9) {
                            if ($.trim($(this).val()) &&
                            (!that.element.siblings('.typeahead').length ||
                                that.element.siblings('.typeahead').is(':hidden'))) {
                                ...
                                if (!that.options.autocompleteOnComma) {
                                    ...
                                    that.process()
                                } else if (!that.element.siblings('.typeahead').length ||
                                    that.element.siblings('.typeahead').is(':hidden')) {
                                    event.preventDefault()
                                } else {
                ...
                .typeahead({
                    ...
                    matcher: function(value) {
                        return ~value.toLowerCase().indexOf(this.query.toLowerCase()) &&
                            (that.inValues(value) == -1 || that.options.allowDuplicates)
                    },
        ...
        , createBadge: function (value) {
            var that = this;
            var text = this.options.searchlink
                ? (this.options.showSearchIcon
                    ? $('<span><a class="search notajax" target="_blank" href="' + this.options.searchlink + value.toString() + '" ><span class="fa fa-search"></span></a>' + value.toString() + '</span>')
                    : $('<a class="notajax white" target="_blank" href="' + this.options.searchlink + value.toString() + '" >' + value.toString() + '</a>'))
                : value.toString();
        ...
        , add: function (value) {
            ...
            if (that.options.denyNewTags) {
                var suggestions = that.input.select().parents(".form-group").find(".typeahead.dropdown-menu li a");
                var alreadyExists = suggestions.filter(function(item) {
                    return suggestions[item].text == value;
                    });
